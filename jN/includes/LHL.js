(function(){
    ...
    var specials = [
        {date:11, type:'bad', name:'待在男（女）友身边', description:'脱团火葬场，入团保平安。'}
    ];

    var tools = ["Eclipse写程序", "MSOffice写文档", "记事本写程序", "Windows8", "Linux", "MacOS", "Firefox", "Android设备", "iOS设备", "Notepad++", 'Vim'];
    ...
    function pickSpecials() {
        ...
        for (var i = 0; i < specials.length; i++) {
            ...
            if (('' + iday).indexOf(special.date) == 6) {
    ...
    function addToGood(event) {
        goodhtml += '<li><div class="name">' + event.name + '</div><div class="description">' + event.good + '</div></li>';
    }

    // 添加到“不宜”
    function addToBad(event) {
        badhtml += '<li><div class="name">' + event.name + '</div><div class="description">' + event.bad + '</div></li>'
    }

    jN.menu.addItem({
        text: '程序员老黄历',
        cmd:function(){
            var html = '<div class="container"><div class="date">' + getTodayString() + '</div>';
            pickTodaysLuck();

            html += '<div class="good"><table><tr><td class="title">宜</td>';
            html += '<td class="content"><ul>' + goodhtml + '</ul></td></tr></table>';
            html += '</div>';

			html += '<div class="split"></div>';

            html += '<div class="bad"><table><tr><td class="title">不宜</td>';
            html += '<td class="content"><ul>' + badhtml + '</ul></td></tr></table>';
			html += '</div>';

            html += '<div class="split"></div>';
			html += '<div class="line-tip">';
            html += '<strong>座位朝向：</strong>面向<span class="direction_value">' + directions[random(iday, 2) % directions.length] + '</span>写程序，BUG 最少。';
			html += '</div>';
			html += '<div class="line-tip">';
            html += '<strong>今日宜饮：</strong><span class="drink_value">' + pickRandom(drinks,2).join('，') + '</span>';
			html += '</div>';
			html += '<div class="line-tip">';
            html += '<strong>女神亲近指数：</strong><span class="goddes_value">' + star(random(iday, 6) % 5 + 1) + '</span>';
			html += '</div>';
            ...
            var dlg = new Dialog({
                npp:Editor,
                title: '程序员老黄历',
                css: 'body *{font-family:"Consolas","Microsoft Yahei",Arial,sans-serif}body{background:white;margin:0;padding:0}.container{width:420px;margin:0 auto 50px}.container>.title{color:#bbb;font-weight:bold;margin-bottom:10px;background:#555;padding:5px 15px}.date{font-size:15pt;font-weight:bold;line-height:30pt;text-align:center}.split,.clear{clear:both;height:1px;overflow-y:hidden}.good,.bad{clear:both;position:relative}.good .title,.bad .title{width:100px;font-weight:bold;text-align:center;font-size:30pt;}.good .title{background:#fe4}.bad .title{background:#f44;color:#fff}.good .content,.bad .content{margin-left:115px;padding-right:10px;padding-top:1px;font-size:15pt} .good{background:#ffa}.bad{background:#ffddd3}.content ul{list-style:none;margin:10px 0 0;padding:0}.content ul li{line-height:150%;font-size:15pt;font-weight:bold;color:#444}.content ul li div.description{font-size:11pt;font-weight:normal;color:#777;line-height:110%;margin-bottom:10px}.line-tip{font-size:11pt;margin-top:10px;margin-left:10px}.direction_value{color:#4a4;font-weight:bold}.goddes_value{color:#f87}',
                html: html,
                width:440,
                clientWidth:440,
                height:300,
